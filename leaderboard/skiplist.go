...
func less(a, b Entry) bool {
    if a.Score == b.Score { return a.User < b.User }
...
func (s *SkipList) Update(user core.UserID, score int64) {
    ...
    lvl := s.randomLevel()
    if lvl > s.lvl {
        for i := s.lvl; i < lvl; i++ { update[i] = s.head }
        s.lvl = lvl
    }
    n := &node{e: e}
    for i := 0; i < lvl; i++ {
...
func (s *SkipList) removeLocked(user core.UserID, e Entry) {
    ...
    if target == nil || target.e.User != user { return }
    for i := 0; i < s.lvl; i++ {
    ...
    for s.lvl > 1 && s.head.next[s.lvl-1] == nil { s.lvl-- }
}
...
func (s *SkipList) Get(user core.UserID) (Entry, bool) {
    s.mu.RLock(); defer s.mu.RUnlock()
    if n, ok := s.byUser[user]; ok { return n.e, true }
    return Entry{}, false
}
